...
# 拉取镜像（离线版：先查本地是否已由 import-resources.sh 导入，有则跳过）
echo "检查 kube-vip 镜像: $KUBEVIP_IMAGE"
if ctr -n k8s.io images ls -q 2>/dev/null | grep -q "kube-vip"; then
    echo "[=] kube-vip 镜像已在本地，跳过拉取"
else
    echo "本地无 kube-vip，尝试拉取..."
    ctr -n k8s.io images pull "$KUBEVIP_IMAGE" || {
        echo "[!] 拉取失败，尝试离线导入..."
        if [ -f "/opt/k8s-deploy/images/kube-vip.tar" ]; then
            ctr -n k8s.io images import "/opt/k8s-deploy/images/kube-vip.tar"
        elif [ -f "${SCRIPT_DIR}/../images/kube-vip.tar" ]; then
            ctr -n k8s.io images import "${SCRIPT_DIR}/../images/kube-vip.tar"
        else
...
# --- 4. 预拉取镜像（离线版：先检查本地是否已有，有则跳过联网拉取）---
echo ""
echo "--- 检查 K8s 镜像 ---"
MISSING_IMG=0
for img in $(kubeadm config images list --config /etc/kubernetes/kubeadm-config.yaml 2>/dev/null); do
    if ! ctr -n k8s.io images ls -q 2>/dev/null | grep -q "$img"; then
        MISSING_IMG=1; break
    fi
done
if [ "$MISSING_IMG" -eq 0 ]; then
    echo "[=] 所有 K8s 镜像已在本地（import-resources.sh 已导入），跳过预拉取"
else
    echo "本地缺镜像，尝试拉取..."
    kubeadm config images pull --config /etc/kubernetes/kubeadm-config.yaml || \
        echo "[!] 预拉取失败（离线正常，请确认 import-resources.sh 已执行）"
fi
