...
# --- 登录私有仓库 ---
REGISTRY_HOST=$(echo "${PRIVATE_REGISTRY}" | cut -d/ -f1)
echo "--- 登录私有仓库 ${REGISTRY_HOST} ---"
if command -v docker &>/dev/null && systemctl is-active --quiet docker 2>/dev/null; then
    CTL="docker"
    docker login "${REGISTRY_HOST}" || { echo "[✗] docker login 失败"; exit 1; }
else
    CTL="nerdctl"
    command -v nerdctl &>/dev/null || { echo "[✗] 需要 docker 或 nerdctl 来推送镜像"; exit 1; }
    nerdctl login "${REGISTRY_HOST}" || { echo "[✗] nerdctl login 失败"; exit 1; }
fi
echo "[✓] 使用 ${CTL} 推送"
...
for img in $K8S_SRC; do
    # registry.aliyuncs.com/google_containers/X:tag → PRIVATE_REGISTRY/X:tag
    PAIRS+=("${img}|${PRIVATE_REGISTRY}/${img##*/}")
done
# kube-vip: ghcr → PRIVATE_REGISTRY/kube-vip/kube-vip:tag
PAIRS+=("ghcr.io/kube-vip/kube-vip:${KUBEVIP_VERSION}|${PRIVATE_REGISTRY}/kube-vip/kube-vip:${KUBEVIP_VERSION}")
# calico: DaoCloud → PRIVATE_REGISTRY/calico/X:tag
for c in node cni kube-controllers; do
    PAIRS+=("m.daocloud.io/docker.io/calico/${c}:${CALICO_VERSION}|${PRIVATE_REGISTRY}/calico/${c}:${CALICO_VERSION}")
done
...
for pair in "${PAIRS[@]}"; do
    ...
    if [ "$MODE" == "pull" ]; then
        ${CTL} pull "$src" 2>/dev/null || { echo "  [✗] 拉取失败"; FAIL=$((FAIL+1)); continue; }
    fi
    ${CTL} tag "$src" "$tgt" 2>/dev/null || { echo "  [✗] 打标签失败（本地无此镜像？试 --pull）"; FAIL=$((FAIL+1)); continue; }
    ${CTL} push "$tgt" 2>/dev/null || { echo "  [✗] 推送失败"; FAIL=$((FAIL+1)); continue; }
    echo "  [✓] 已推送"
done
